...
def switch_tariff(target_tariff):
	...
	print("Logged in")
        page.wait_for_timeout(1000)
        # replace with env
	page.goto(f"https://octopus.energy/dashboard/new/{config.ACC_NUMBER}/set-auth-token?redirect={target_tariff.upper()}_SIGN_UP&accountNumber={config.ACC_NUMBER}")
	print("Retreived Auth Token")
	page.wait_for_timeout(5000)
        page.goto(f"https://octopus.energy/smart/{target_tariff.lower()}/sign-up/?accountNumber={config.ACC_NUMBER}")
	print("Loaded tariff page")
        page.wait_for_timeout(10000)
        page.locator("section").filter(has_text="Already have a SMETS2 or “").get_by_role("button").click(force=True)
	print("Executed tariff swap")
        page.wait_for_timeout(10000)
